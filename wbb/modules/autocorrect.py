...
async def autocorrect_bot(_, message: Message):
    ...
    if not data.ok:
        return await message.reply_text("Có điều gì đó không ổn đã xảy ra.")
    result = data.result
    await message.reply_text(result.corrected if result.corrected else "Trống")
...
async def autocorrect_ubot_toggle(_, message: Message):
    ...
    if len(message.command) != 2:
        return await eor(message, text="Không đủ lập luận.")
    ...
    if state == "enable":
        ...
        return await eor(
            message, text="Lập luận sai, Pass (ENABLE|DISABLE)."
        )
